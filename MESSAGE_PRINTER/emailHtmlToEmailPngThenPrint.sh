...
/usr/bin/wkhtmltoimage --width "$X_LINES_POS" --quality 100 "$SRC_HTML_HEADER" "$DST_PNG_HEADER" 2> /dev/null &&
# 2> /dev/null
...
# wkhtmltoimage hat ein internes Problem welches Fehler meldet, diese koennen ignoriert werden, muessen aber nach /dev/null
/usr/bin/wkhtmltoimage --height "$X_LINES_POS" --quality 100 "$SRC_HTML_BODY" "$DST_PNG_BODY" 2> /dev/null &&
# 2> /dev/null
